    ...
    def create_seller_offer_template(self, request_data):
        ...
        if not items:
            ...
            data = {
                ...
                'Миқдори': [item['quantity'] for item in items],
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            # Выделяем поля для заполнения цен (желтые)
            for row in range(2, len(items) + 2):
                for col in [5, 6]:  # колонки нархи и Суммаси
                    cell = worksheet.cell(row=row, column=col)
                    cell.fill = cell.fill.copy(fill_type='solid', fgColor='FFFF00')
